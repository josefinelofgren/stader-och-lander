...
export function printCountries(list) {
  ...
  for (let country in countries) {
    if (countries.hasOwnProperty(country)) {
      document
        .getElementById("listCountries")
        .insertAdjacentHTML(
          "beforeend",
          "<ul><li class='listCountries' id='" +
      ...
      for (let city in cities) {
        if (evt.target.id == cities[city].countryid) {
          listCities.insertAdjacentHTML(
            "beforeend",
            "<li class='listCities' id='" +
              cities[city].id +
              "' value='" +
              cities[city].population +
              "'>" +
              cities[city].stadname +
              "</li>"
          );
        }
      }
...
export function printCityInformation() {
  listCities.addEventListener("click", function (evt) {
    ...
    document.getElementById("cityName").innerHTML =
      "<h1 class='city-name' id=" + evt.target.id + ">" + cityTitle + "</h1>";
    document.getElementById("cityInfo").innerHTML =
      "<p class='population'> Antal invånare: " + cityPopulation + "</p>";
  });
}
